...
from shanghai_loader import build_shanghaitech_lists
...
def main():
    ...
    shanghai_seq, shanghai_lbl = build_shanghaitech_lists("ShanghaiTech/")
    ubnormal_seq, ubnormal_lbl = build_ubnormal_lists("UBnormal/")
    penn_seq, penn_lbl, _, _ = build_penn_action_lists("Penn_Action/")
    pretrained_seq = shanghai_seq + ubnormal_seq + penn_seq
    pretrained_lbl = shanghai_lbl + ubnormal_lbl + penn_lbl
    combined = list(zip(pretrained_seq, pretrained_lbl))
    random.seed(42)
    random.shuffle(combined)
    pretrained_seq[:], pretrained_lbl[:] = zip(*combined)
    ...
    if pretrain: 
        ...
        torch.save(model.state_dict(), "action_checkpoints/Combined_pretrained.pt")
    ...
    three_d = False
    t1 = load_T1(
        model_path="action_checkpoints/Combined_pretrained.pt",
        num_joints=NUM_JOINTS_PENN,
        three_d=three_d,
        d_model=hidden_size,
        nhead=n_heads,
